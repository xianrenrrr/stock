...
1. Rank evidence in this order:
   - Fresh company-specific earnings, guidance, offering/dilution, contract/order size, regulatory order/approval, financing, buyback/dividend, or confirmed financial impact.
...
5. Failed gap-up after a rally is bearish:
   - `0.47-0.49` on neutral volume.
...
6. Negative peer earnings reactions, such as AI outlook disappointment, override stale bullish sector narratives for one session.
...
5. Use `0.55+` only for fresh measurable hard catalysts with confirmation and no extension/fade warning.
6. Never use `0.55+` for stale AI, product, analyst, partnership, index, or theme headlines.
7. If evidence is mixed, stale, duplicated, low-volume, or conflicting across retrieved cases, force `0.49-0.51`.
...
5. Do not raise confidence for familiar tickers or retrieved similar cases alone; confidence did not reliably predict accuracy in this review period.
